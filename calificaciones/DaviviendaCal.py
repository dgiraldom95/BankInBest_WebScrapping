...
def obtenerCalificacion():
    ...
    url = 'http://www.brc.com.co/calificaciones_busqueda.php?fecha_desde=&fecha_hasta=&cliente=BANCO+DAVIVIENDA+S.+A.&subsector=&tipo_calificacion=&tipo_revision=&vigencia=&tipo_escala=&Submit=Realizar+B%FAsqueda'
                ...
                if type(td) is bs4.Tag or type(td):
                    if td.string == None:
                        ...
                        if len(resp) != 0 and final != "":
                            for i in resp:
                                resp[i] = final
                            cal = CalificacionBancaria(str('Davivienda'), str(final))
                            break
                        elif len(resp) == 0:
                            resp[final] = ""
